...
task extractDepth {
    ...
    command {
        ...
        cut -f1-4 | \
        bgzip > coverage/${chromosome}.${sample}.depth.gz \
        && tabix -b 2 coverage/${chromosome}.${sample}.depth.gz
    }
    output {
        File outDepth = "coverage/${chromosome}.${sample}.depth.gz"
        File outIndex = "coverage/${chromosome}.${sample}.depth.gz.tbi"
    }
    runtime {
...
task aggrBasePair {
    ...
    command {
        ...
        mkdir -p coverage/full
        merge_coverage.py -i file.list -o coverage/full/${chromosome}.full.json.gz
        mkdir -p coverage/bin_25e-2
        prune_coverage.py -i coverage/full/${chromosome}.full.json.gz -l 0.25 -o coverage/bin_25e-2/${chromosome}.bin_0.25.json.gz
        mkdir -p coverage/bin_50e-2
        prune_coverage.py -i coverage/full/${chromosome}.full.json.gz -l 0.50 -o coverage/bin_50e-2/${chromosome}.bin_0.50.json.gz
        mkdir -p coverage/bin_75e-2
        prune_coverage.py -i coverage/full/${chromosome}.full.json.gz -l 0.75 -o coverage/bin_75e-2/${chromosome}.bin_0.75.json.gz
        mkdir -p coverage/bin_10e-1
        prune_coverage.py -i coverage/full/${chromosome}.full.json.gz -l 1.00 -o coverage/bin_10e-1/${chromosome}.bin_1.00.json.gz
    }
    output {
        File outAggrBasePair = "coverage/full/${chromosome}.full.json.gz"
        File outAggrBasePair_index = "coverage/full/${chromosome}.full.json.gz.tbi"
        File outPruneCov0_25 = "coverage/bin_25e-2/${chromosome}.bin_0.25.json.gz"
        File outPruneCov0_25_index = "coverage/bin_25e-2/${chromosome}.bin_0.25.json.gz.tbi"
        File outPruneCov0_50 = "coverage/bin_50e-2/${chromosome}.bin_0.50.json.gz"
        File outPruneCov0_50_index = "coverage/bin_50e-2/${chromosome}.bin_0.50.json.gz.tbi"
        File outPruneCov0_75 = "coverage/bin_75e-2/${chromosome}.bin_0.75.json.gz"
        File outPruneCov0_75_index = "coverage/bin_75e-2/${chromosome}.bin_0.75.json.gz.tbi"
        File outPruneCov1_00 = "coverage/bin_10e-1/${chromosome}.bin_1.00.json.gz"
        File outPruneCov1_00_index = "coverage/bin_10e-1/${chromosome}.bin_1.00.json.gz.tbi"
    }
    runtime {
        docker: "statgen/bravo-pipeline:latest"
        cpu: "2"
